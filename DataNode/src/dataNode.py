...
def Replicate(filename,folder,file_data,create_folder):
    try:
        with grpc.insecure_channel(f"{HOST}:{REPLICA}",options=[
        ('grpc.max_receive_message_length', 1024 * 1024 * 100)  
    ]) as channel:
            stub = dataNode_apiGateway_pb2_grpc.DataNodeServiceStub(channel)
            response = stub.WriteFile(dataNode_apiGateway_pb2.WriteFileRequestData(file_data=file_data,filename=filename,folder=folder,create_folder=create_folder))
            return response
    except Exception as error:
        print(error)
